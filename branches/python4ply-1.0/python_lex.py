...
# I put this before t_WHITESPACE so it can consume lines with only comments in them.
...
# The original lex token stream contains WHITESPACE and NEWLINE characters.
# WHITESPACE will only occur before any other tokens on a line.

# I have three filters.  One tags tokens by adding two attributes.
# "must_indent" is True if the token must be indented from the
# previous code.  The other is "at_line_start" which is True for WHITESPACE
# and the first non-WHITESPACE/non-NEWLINE on a line.  It flags the check so
...
def annotate_indentation_state(lexer, token_stream):
    ...
    for token in token_stream:
        ...
        if token.type == "DEF":
            #print "DEFFFF"; import sys; sys.exit()
            at_line_start = False
            indent = MAY_INDENT
...
def synthesize_indentation_tokens(token_stream):
    ...
    for token in token_stream:
        ...
        # WHITESPACE only occurs at the start of the line
        # There may be WHITESPACE followed by NEWLINE so
        ...
        if token.type == "WHITESPACE":
            ...
            prev_was_ws = True
            # WHITESPACE tokens are never passed to the parser
            continue
        ...
        # then it must be a real token (not WHITESPACE, not NEWLINE)
...
text = open("example.snow").read()
lexer.input(text, "example.snow")
for tok in lexer:
    print tok
    pass
